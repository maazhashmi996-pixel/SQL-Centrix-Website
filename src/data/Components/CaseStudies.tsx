...
import { caseStudiesData } from "@/src/data/caseStudiesData";
import { Sparkles, ArrowRight, TrendingUp, Award, Zap, CheckCircle2, Layers } from "lucide-react";

// Reusable counter component for metrics inside case studies
function MetricCounter({ value }: { value: string }) {
  // Extract number and suffix/prefix if any (e.g. "+340%", "4.8x", "99.4%")
  ...
  const rounded = useTransform(count, (latest) => {
    // If it has a decimal point like 4.8
    if (value.includes(".")) {
...
export default function CaseStudies() {
  return (
    <section id="case-studies" className="py-32 px-6 lg:px-20 bg-gradient-to-b from-white via-slate-50/80 to-purple-50/20 relative overflow-hidden border-b border-purple-100/60 selection:bg-purple-600 selection:text-white">
      
      {/* Absolute Luxurious Light Ambient Backlights */}
      <div className="absolute top-1/4 -right-20 w-[600px] h-[600px] bg-gradient-to-tr from-purple-300/30 via-indigo-200/20 to-transparent blur-[140px] rounded-full pointer-events-none" />
      <div className="absolute bottom-10 -left-20 w-[500px] h-[500px] bg-gradient-to-br from-purple-200/40 via-pink-100/20 to-transparent blur-[140px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          {/* Left Column: Sticky Header & Elite Light-Theme Action Card */}
          <div className="lg:col-span-4 space-y-8 lg:sticky lg:top-32">
            ...
            <motion.div 
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: 0.2 }}
              className="p-8 rounded-[2.5rem] bg-gradient-to-br from-slate-900 via-purple-950 to-indigo-950 text-white space-y-6 shadow-2xl shadow-purple-950/20 relative overflow-hidden border border-purple-500/20 backdrop-blur-xl group"
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-purple-500/20 to-transparent rounded-bl-full pointer-events-none group-hover:scale-110 transition-transform duration-500" />
              <div className="absolute -right-6 -bottom-6 w-36 h-36 bg-indigo-500/20 rounded-full blur-2xl pointer-events-none group-hover:scale-125 transition-transform duration-500" />
              
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-purple-400 via-indigo-400 to-purple-300" />
              ...
              <div className="space-y-2.5 pt-2 border-t border-purple-800/60">
              ...
              <motion.a 
                ...
                whileTap={{ scale: 0.98 }}
                href="#contact"
                className="inline-flex items-center justify-center gap-2 w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-purple-600 via-indigo-600 to-violet-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold text-sm shadow-lg shadow-purple-900/30 transition-all duration-300 group/btn border border-purple-400/30 cursor-pointer"
              >
          ...
          {/* Right Column: Case Studies Cards Grid with Premium Light Style */}
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            {caseStudiesData.map((cs, idx) => (
              <CaseStudyCard key={idx} cs={cs} idx={idx} />
            ))}
  ...
  return (
    <div ref={ref} className="space-y-4 text-left">
      <motion.div 
        ...
        animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
        transition={{ duration: 0.5 }}
        className="inline-flex items-center gap-2 px-4.5 py-2 rounded-full bg-purple-100/80 border border-purple-200 text-purple-800 text-xs font-bold uppercase tracking-wider shadow-sm backdrop-blur-md"
      >
        <Sparkles className="w-3.5 h-3.5 text-purple-600 animate-pulse" />
      ...
      <h2 className="text-3xl sm:text-5xl font-black text-slate-900 tracking-tight leading-[1.12]">
        ...
        Built for <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-800 font-serif italic font-normal tracking-normal">Global</span> Partners.
      </h2>
...
// --- Case Study Card Component (Light Theme & Premium Unique View) ---
function CaseStudyCard({ cs, idx }: { cs: any; idx: number }) {
  const cardRef = useRef<HTMLDivElement>(null);
  const isInView = useInView(cardRef, {
    margin: "-20% 0px -20% 0px",
  ...
  return (
    <motion.div 
      ...
      className="bg-white/90 hover:bg-white rounded-[2.2rem] p-7 border border-purple-100/80 shadow-[0_10px_30px_-10px_rgba(147,51,234,0.05)] backdrop-blur-xl flex flex-col justify-between space-y-6 group transition-all duration-300 relative overflow-hidden text-left"
    >
      {/* Top Gradient Accent Line */}
      <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-500 opacity-70 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-mono font-bold text-purple-400 bg-purple-50/50 px-2.5 py-1 rounded-lg border border-purple-100">
            #{String(idx + 1).padStart(2, '0')}
          </span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 py-4 px-4 border border-purple-100/60 bg-gradient-to-br from-purple-50/50 via-slate-50 to-indigo-50/30 rounded-2xl relative overflow-hidden">
        <div className="absolute inset-0 bg-white/40 pointer-events-none" />
        {cs.metrics.map((m: any, mIdx: number) => (
          <div key={mIdx} className="space-y-1 relative z-10">
            <div className="text-xl font-black text-purple-950 flex items-center gap-1 tracking-tight font-mono">
              <TrendingUp className="w-4 h-4 text-purple-600 shrink-0" />
      ...
      <motion.a
        ...
        whileTap={{ scale: 0.98 }}
        href="#contact"
        className="w-full text-center flex items-center justify-center gap-2 bg-slate-900 hover:bg-gradient-to-r hover:from-purple-600 hover:to-indigo-600 text-white font-bold py-3.5 px-4 rounded-2xl transition-all duration-300 text-xs shadow-lg shadow-slate-900/10 group/btn cursor-pointer"
      >
