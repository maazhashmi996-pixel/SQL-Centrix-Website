...
export default function Navbar() {
  ...
  return (
    <header className={`sticky top-0 z-50 w-full transition-all duration-300 ${
      scrolled 
        ? "bg-white/95 backdrop-blur-md border-b border-indigo-500/20 shadow-[0_10px_30px_rgba(99,102,241,0.08)] py-3" 
        : "bg-white border-b border-indigo-500/10 py-2"
    }`}>
      {/* Full Row Container */}
      <div className="w-full px-6 sm:px-6 lg:px-12 flex items-center justify-between">
        ...
        <div className="flex items-center">
          <a href="#" className="flex items-center gap-3.5 focus:outline-none group">
            <div className="relative p-1 rounded-2xl bg-gradient-to-tr from-indigo-600 via-purple-600 to-pink-600 shadow-[0_0_15px_rgba(99,102,241,0.25)] group-hover:shadow-[0_0_25px_rgba(99,102,241,0.4)] transition-all duration-300 group-hover:scale-105">
              <div className="bg-white px-3.5 py-2.5 rounded-[14px] flex items-center justify-center overflow-hidden">
                <img
                  src="logo2.jpg" 
                  alt="SQL Centrix Logo" 
            ...
            <div className="flex flex-col">
              <span className="text-base font-black tracking-tight text-slate-900 flex items-center gap-1">
                SQL <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600">Centrix</span>
              </span>
              <span className="text-[10px] uppercase font-bold tracking-widest text-indigo-600 -mt-0.5 flex items-center gap-1">
        ...
        <nav className="hidden md:flex items-center space-x-8 text-sm font-medium text-slate-700">
          {navLinks.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="transition-colors hover:text-indigo-600 py-1"
            >
              {item.name}
            </a>
          ))}
        ...
        <div className="hidden md:block">
          <motion.a
            ...
            className="inline-flex items-center gap-2 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-700 hover:to-pink-700 text-white text-xs font-bold uppercase tracking-wider px-5 py-2.5 rounded-xl transition-all duration-300 border border-indigo-300/40 shadow-lg"
          >
            <span>Get Started</span>
            <ArrowRight className="w-3.5 h-3.5" />
          </motion.a>
        ...
        <div className="md:hidden flex items-center">
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="p-2 rounded-xl bg-slate-100 border border-indigo-500/30 text-slate-800 hover:bg-slate-200 transition-colors focus:outline-none"
            aria-label="Toggle Menu"
          >
            {isOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
          </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, height: 0 }}
            transition={{ duration: 0.25 }}
            className="md:hidden w-full bg-white border-b border-indigo-500/20 overflow-hidden px-6 py-5 shadow-xl"
          >
            <nav className="flex flex-col space-y-3 text-sm font-medium text-slate-700">
              {navLinks.map((item) => (
                <a
                  ...
                  href={item.href}
                  onClick={() => setIsOpen(false)}
                  className="px-3 py-2 rounded-lg hover:bg-indigo-50 hover:text-indigo-600 transition"
                >
                  {item.name}
                </a>
              ))}
              ...
              <div className="pt-2">
                <a
                  href="#contact"
                  onClick={() => setIsOpen(false)}
                  className="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white text-xs font-bold uppercase tracking-wider py-3 rounded-xl transition-all border border-indigo-300/40 shadow-lg"
                >
                  <span>Get Started</span>
                  <ArrowRight className="w-3.5 h-3.5" />
                </a>
