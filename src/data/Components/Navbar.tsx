...
//                                 }
//                                 className="group flex flex-col rounded-xl p-2.5 transition-all hover:bg-yellow-50"
//                               >
...
//                                 <span className="mt-0.5 pl-4 text-[10px] text-slate-500">
//                                   {service.desc}
//                                 </span>
//                               </a>
//                             ))}
...
//   // Default state set to display admin button smoothly on all routes/dashboard
//   const [admin, setAdmin] = useState<{ name: string } | null>({
//     name: "Admin",
//   });
...
//                 src="/logo2.jpg"
//                 alt="SQL Centrix Logo"
//                 className="h-29 w-[167px] bg-transparent"
//               />
//             </div>
//           </motion.a>
//         </div>
...
//           {/* Admin Badge Section (With Natural Yellow Border) */}
//           {admin && (
//             <motion.a
//               href="/admin/dashboard"
//               whileHover={{ y: -2, scale: 1.02 }}
//               whileTap={{ scale: 0.97 }}
//               className="flex items-center gap-2 rounded-xl border border-yellow-400 bg-white px-3.5 py-2 text-xs font-bold text-slate-900 transition-all hover:bg-yellow-50 hover:shadow-md"
//             >
//               <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-yellow-400 bg-yellow-400 text-slate-950 shadow-sm">
...
//             <a
//               href="/admin/dashboard"
//               className="flex h-11 items-center gap-1.5 rounded-xl border border-yellow-400 bg-white px-2.5 text-xs font-bold text-slate-900 shadow-sm"
//             >
//               <div className="flex h-6 w-6 items-center justify-center rounded-md bg-yellow-400 text-slate-950">
//                 <User className="h-3.5 w-3.5" />
//               </div>
//               <span className="max-w-[60px] truncate">{admin.name}</span>
//             </a>
//           )}
...
//                     <p className="text-[8px] text-slate-500">Growth Partner</p>
//                   </div>
//                 </div>
...
export default function Navbar() {
  ...
  // Default login state false rakhi hai taakay starting me sirf icon show ho, text show na ho
  const [admin, setAdmin] = useState<{ isLoggedIn: boolean; name?: string }>({
    isLoggedIn: false,
    name: "",
  });
  ...
  useEffect(() => {
    ...
    if (storedAdmin) {
      try {
        ...
        setAdmin({ isLoggedIn: true, name: parsed.name || "Admin" });
      } catch {
        setAdmin({ isLoggedIn: true, name: "Admin" });
      }
    }
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-white transition-all duration-500">
      ...
      <div className="mx-auto flex w-full max-w-[1240px] items-center justify-between px-4 sm:px-6">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <motion.a
            href={admin.isLoggedIn ? "/admin/dashboard" : "/admin/login"}
            whileHover={{ y: -2, scale: 1.02 }}
            whileTap={{ scale: 0.97 }}
            className="flex items-center gap-2 rounded-xl border border-yellow-400 bg-white p-2 text-xs font-bold text-slate-900 transition-all hover:bg-yellow-50 hover:shadow-md"
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-yellow-400 bg-yellow-400 text-slate-950 shadow-sm">
              <User className="h-4 w-4" />
            </div>

            {/* Admin Text Sirf tabhi Render hoga jab admin logged in ho */}
            {admin.isLoggedIn && admin.name && (
              <span className="truncate max-w-[100px] pr-1.5">{admin.name}</span>
            )}
          </motion.a>
        </div>
        ...
        <div className="flex items-center gap-2 md:hidden">
          <a
            href={admin.isLoggedIn ? "/admin/dashboard" : "/login"}
            className="flex h-11 items-center gap-1.5 rounded-xl border border-yellow-400 bg-white p-2 text-xs font-bold text-slate-900 shadow-sm"
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-md bg-yellow-400 text-slate-950">
              <User className="h-4 w-4" />
            </div>
            {admin.isLoggedIn && admin.name && (
              <span className="max-w-[60px] truncate pr-1">{admin.name}</span>
            )}
          </a>
